...
- `streamName`: Redis stream name

If the stream entry includes a non-empty `correlationId` field, it is preserved
on the message envelope (`message.correlationId`). A missing value is **not**
generated by this input, and `metadata.correlationId` is not auto-populated.
If you need generated correlation IDs for tracing, configure the
